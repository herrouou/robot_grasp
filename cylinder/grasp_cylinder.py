...
# =============================================================================
# CYLINDER GRASPING CONTROL SYSTEM
# =============================================================================
...
class CylinderGraspingController:
    ...
    def __init__(self, model, data):
        """
        Initialize the cylinder grasping controller
    ...
    def init_controllers(self):
        ...
        # Finger controllers - same gains as original but organized
    ...
    def calculate_cylinder_target_positions(self):
        """
        Calculate target contact positions for cylinder grasping
        Uses angled approach with contact offsets for stable cylinder grip
        """
    ...
    def control_initial_positioning(self):
        """Control phase 2: Position hand near cylinder object"""
        hand_pos = self.data.body('palm').xpos
        target_pos = self.data.body('cylinder_object').xpos
        
        # Calculate desired offset from cylinder (different from box)
    ...
    def control_cylinder_grasping(self):
        """Control phase 3: Execute cylinder grasping motion"""
        ...
        # Optional: Create combined Jacobian for coordinated control
        self.jacobi_all = np.vstack([
            self.ff_contact_jacp, self.mf_contact_jacp, 
            self.rf_contact_jacp, self.th_contact_jacp
        ])
        self.control_signal_all = np.vstack([
    ...
    def compute_cylinder_joint_space_control(self, ff_control, mf_control, rf_control, th_control):
        ...
        with cylinder-specific null space control for joint limit avoidance
        """
        ...
        # Note: Fixed bug in original code - rf should use rf_contact_jacp, not ff_contact_jacp
    ...
    def control_lifting(self):
        """Control phase 4: Lift the grasped cylinder"""
    ...
    def print_debug_info(self):
        ...
        print("Grasp targets:")
        print("  ff_target:", self.ff_contact_target)
        print("  mf_target:", self.mf_contact_target)
        print("  rf_target:", self.rf_contact_target)
        print("  th_target:", self.th_contact_target)
...
def main():
    """Main simulation function for cylinder grasping"""
    # Load MuJoCo model and initialize data
    model = mujoco.MjModel.from_xml_path(CylinderGraspConfig.XML_PATH)
    ...
    # Initialize cylinder grasping controller
    controller = CylinderGraspingController(model, data)

    # Debug: Print initial contact information
    print("Initial contacts:", data.contact)
    print("Cylinder grasping configuration loaded")
    ...
    with mujoco.viewer.launch_passive(model, data) as viewer:
        ...
        while viewer.is_running() and time.time() - start < 1000:
            ...
            # Update viewer with contact point visualization
            with viewer.lock():
